...
void LSM9DS1::update()
{
    ...
    temperature = (double)(((int16_t)response[1] << 8) | response[0]) / 256. + 25.;
    ...
    _ax = G_SI * ((double)bit_data[0] * acc_scale);
    _ay = G_SI * ((double)bit_data[1] * acc_scale);
    _az = G_SI * ((double)bit_data[2] * acc_scale);
    ...
    _gx = (PI / 180) * ((double)bit_data[0] * gyro_scale);
    _gy = (PI / 180) * ((double)bit_data[1] * gyro_scale);
    _gz = (PI / 180) * ((double)bit_data[2] * gyro_scale);
    ...
    _mx = 100.0 * ((double)bit_data[0] * mag_scale);
    _my = 100.0 * ((double)bit_data[1] * mag_scale);
    _mz = 100.0 * ((double)bit_data[2] * mag_scale);
...
void LSM9DS1::rotate()
{
    double replacement_acc, replacement_gyro;
